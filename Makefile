...
REPO    := github.com/tidbcloud/tidb2snowflake
...
# End-to-end tests against a real TiDB Cloud cluster + Snowflake + S3.
# Requires the E2E_* environment variables (see docs/e2e.md); skips otherwise.
.PHONY: e2e
e2e:
	CGO_ENABLED=$(CGO_ENABLED) go test -tags e2e -ldflags '$(LDFLAGS)' -timeout 40m -v ./test/e2e/...
