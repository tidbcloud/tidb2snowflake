...
`COPY INTO` 和 `MERGE INTO` 都走 Go `database/sql` 的 `db.Exec`，**同步阻塞到 Snowflake 真正落表并提交才返回**（`pkg/snowsql/sql.go:64`、`pkg/snowsql/connector.go:106`；全仓库无 async 模式）。tidb2snowflake **只在 `Exec` 成功返回后才写 `loadinfo`/`.checkpoint` 标记**，正是这个同步语义让「标记 = 已落表」成立，从而支撑幂等与断点续传。**这是正确性地基，不能改成异步。** 优化空间在装载**策略**，不在同步性。
